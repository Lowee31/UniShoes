    ...
    <header className="fixed z-50 w-screen p-3 px-4 md:p-6 md:px-16 bg-primary">
      ...
      <div className="hidden md:flex w-full h-full items-center justify-between">
        <Link to={'/'} className="flex items-center gap-2">
          <img src={Logo} className="w-15 h-12 object-cover" alt="logo" />
          <p className="textheadingColor text-l text-orange-600">UNISHOES</p>
        </Link>
            ...
            />
            {
              isShoes && (
                <motion.div
                  ...
                  className="w=50 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute top-12 right-0 px-2 py-2">
                  {
                    user && user.email === "johnlouiecampos18@gmail.com" && (
                      <Link to={"/createItem"}>
                        <p className="px-1 py-2 flex items-center gap-3 cursor-pointer  hover:bg-slate-100 
                    transition-all duration-100 ease-in-out text-textColor text-base"
                          onClick={() => setisShoes(false)}
                        >
                          New Item <MdAdd />
                        </p>
                      </Link>
      ...
      <div className="flex items-center justify-between md:hidden w-full h-full">
        ...
        <Link to={'/'} className="flex items-center gap-2">
          <img src={Logo} className="w-15 h-12 object-cover" alt="logo" />
          <p className="textheadingColor text-l text-orange-600"></p>
        </Link>
          ...
          />
          {
            isShoes && (
              <motion.div
                ...
                <p className="m-2 p-3 rounded-md shadow-md flex items-center justify-center bg-gray-200 gap-3 cursor-pointer hover:bg-gray-300 
                transition-all duration-100 ease-in-out text-textColor text-base"
                  onClick={logout}
                >
                  Logout <MdLogout />
